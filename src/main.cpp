                                        ...
                                        const VkRenderPass render_pass)
{
  ...
  for (size_t i = 0; i < swapchain_image_views.size(); i++) {
    ...
    VKH_CHECK(vkCreateFramebuffer(device, &framebuffer_info, nullptr,
...
create_command_buffers(const VkDevice device, const VkCommandPool command_pool,
                       const VkRenderPass render_pass,
                       const VkPipeline pipeline, const Swapchain& swapchain,
...
void draw_frame(const GPUDevice& gpu_device, const Swapchain& swapchain,
                ...
                const std::vector<VkCommandBuffer>& command_buffers) noexcept
{
  ...
  const auto device = gpu_device.vk_device();
  ...
  VKH_CHECK(vkQueueSubmit(gpu_device.graphics_queue(), 1, &submitInfo,
  ...
  vkQueuePresentKHR(gpu_device.present_queue(), &presentInfo);
...
auto main() -> int
{
  ...
  GPUDevice gpu_device(window);
  Swapchain swapchain(gpu_device);

  auto render_pass = create_render_pass(gpu_device.vk_device(), swapchain);
  auto [pipeline_layout, pipeline] =
      create_graphics_pipeline(gpu_device.vk_device(), swapchain, render_pass);
  auto framebuffers =
      create_frame_buffers(gpu_device.vk_device(), swapchain, render_pass);

  auto command_pool = create_command_pool(gpu_device.vk_device(),
                                          gpu_device.queue_family_indices());
  auto command_buffers =
      create_command_buffers(gpu_device.vk_device(), command_pool, render_pass,
                             pipeline, swapchain, framebuffers);
  ...
  for (uint32_t i = 0; i < max_frames_in_flight; i++) {
    VKH_CHECK(vkCreateSemaphore(gpu_device.vk_device(), &semaphore_create_info,
                                nullptr, &image_available_semaphores[i]));
    VKH_CHECK(vkCreateSemaphore(gpu_device.vk_device(), &semaphore_create_info,
                                nullptr, &render_finished_semaphores[i]));
    VKH_CHECK(vkCreateFence(gpu_device.vk_device(), &fence_info, nullptr,
                            &in_flight_fence[i]));
  }

  while (!window.should_close()) {
    draw_frame(gpu_device, swapchain, in_flight_fence, image_in_flight_fence,
  ...
  vkDeviceWaitIdle(gpu_device.vk_device());

  for (size_t i = 0; i < max_frames_in_flight; i++) {
    vkDestroySemaphore(gpu_device.vk_device(), render_finished_semaphores[i],
                       nullptr);
    vkDestroySemaphore(gpu_device.vk_device(), image_available_semaphores[i],
                       nullptr);
    vkDestroyFence(gpu_device.vk_device(), in_flight_fence[i], nullptr);
  }

  vkDestroyCommandPool(gpu_device.vk_device(), command_pool, nullptr);

  for (auto framebuffer : framebuffers) {
    vkDestroyFramebuffer(gpu_device.vk_device(), framebuffer, nullptr);
  }

  vkDestroyPipeline(gpu_device.vk_device(), pipeline, nullptr);
  vkDestroyPipelineLayout(gpu_device.vk_device(), pipeline_layout, nullptr);
  vkDestroyRenderPass(gpu_device.vk_device(), render_pass, nullptr);
}
